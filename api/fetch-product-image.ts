...
/** 與 Storage bucket 上限一致（migration 107） */
...
async function downloadProductImage(imageUrl: string): Promise<{ buffer: Buffer; contentType: string }> {
  ...
  const raw = await readResponseWithLimit(res, MAX_UPLOAD_BYTES)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (body.action === 'import') {
      ...
      const { buffer, contentType } = await downloadProductImage(imageUrl)
