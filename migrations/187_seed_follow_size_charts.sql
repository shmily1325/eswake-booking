...
-- 先跑 186，並在後台「＋ 新增尺寸表」上傳 8 張（名稱需一致，年份寫在名稱裡），再跑本檔。
-- 可重跑。
-- =============================================================
...
FROM size_charts s
WHERE p.is_active AND p.brand ILIKE 'Follow' AND s.brand = 'Follow'
  AND s.name = '男背心 Men''s Vest 2027'
  AND p.model IN ('ANTHEM P1', 'AFFIX', '015Y P1', 'RESIN', 'SECTION');

UPDATE products p
SET size_chart_id = s.id, updated_at = now()
FROM size_charts s
WHERE p.is_active AND p.brand ILIKE 'Follow' AND s.brand = 'Follow'
  AND s.name = '女背心 Women''s Vest 2027'
  AND p.model IN (
