...
-- 先檢查會刪除多少筆資料（預覽）
-- SELECT 
--   (SELECT COUNT(*) FROM bookings WHERE start_at < '2025-12-04') as bookings_count,
...
--    WHERE related_booking_id IN (SELECT id FROM bookings WHERE start_at < '2025-12-04')) as transactions_count,
...
WHERE related_booking_id IN (
  SELECT id FROM bookings WHERE start_at < '2025-12-04'
);
...
DELETE FROM bookings WHERE start_at < '2025-12-04';

-- 3. 清理舊的 audit_log
DELETE FROM audit_log WHERE created_at < '2025-12-04';

-- 4. 刪除 daily_tasks 表（沒有在使用）
DROP TABLE IF EXISTS daily_tasks CASCADE;

-- 5. 保留每日公告/交辦事項（daily_announcements）- 不刪除
-- DELETE FROM daily_announcements WHERE display_date < '2025-12-04';

-- =============================================
-- 驗證清理結果
-- =============================================
-- SELECT 
--   (SELECT COUNT(*) FROM bookings WHERE start_at < '2025-12-04') as remaining_old_bookings,
...
-- 5. 每日公告/交辦事項（daily_announcements）會保留
