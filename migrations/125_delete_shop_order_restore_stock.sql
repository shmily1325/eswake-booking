-- 125: 刪除訂單時還原庫存（待結帳釋放 reserve、已結帳加回 stock）
-- 儲值交易保留（shop_order_id ON DELETE SET NULL），與規劃 §5.4 一致
...
BEGIN
  ...
  DELETE FROM shop_orders WHERE id = p_order_id;
...
GRANT EXECUTE ON FUNCTION public.delete_shop_order(UUID, TEXT) TO authenticated, anon;

SELECT '125_delete_shop_order_restore_stock: delete_shop_order restores reserve + stock' AS status;
