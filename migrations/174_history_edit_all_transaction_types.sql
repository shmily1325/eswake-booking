-- =============================================================================
-- 174_history_edit_all_transaction_types.sql
--
-- 1) 歷史交易編輯／刪除不再限制 transaction_type = 'adjust'
--    （金流與預約／訂單獨立；不改 booking／shop／transaction_type）
-- 2) 沒標年的船券／VIP：若該會員該類別「剛好只有一年」明細，
--    自動動那一年以維持 Σ credit_lots = members；跨年則要求選入帳年
-- =============================================================================

BEGIN;

-- 無標年時：剛好「一年有剩額」→該年；零筆有剩額／跨年有剩額→NULL
CREATE OR REPLACE FUNCTION public.resolve_credit_lot_year_for_untagged(
  p_member_id uuid,
  p_category text
)
RETURNS integer
...
BEGIN
  IF p_category NOT IN (
    'vip_voucher', 'boat_voucher_g23', 'boat_voucher_g21_panther'
  ) THEN
    RETURN NULL;
  END IF;

  -- 只看有剩額的年（remaining=0 的空年不算跨年）
  SELECT COUNT(*), MAX(voucher_year)
  INTO v_count, v_year
  FROM credit_lots
...
COMMENT ON FUNCTION public.resolve_credit_lot_year_for_untagged(uuid, text) IS
  '無標年差額：剛好一年有剩額回傳該年；無剩額或跨年有剩額回傳 NULL。';

-- 當「有剩額的年」不存在、但表上有 remaining=0 的空年時，
-- 退舊帳／刪除仍需要一個落點：用 MIN(voucher_year) 當 fallback（與 restore 一致）
-- （上方 resolve 已處理「剛好一年有剩額」）

-- 跨年（兩年以上有剩額）又沒選年 → 明確失敗（不要猜）
-- 有年帳列但目前都沒剩額（例如 2025=0,2026=0）又選無 → 也失敗，避免總額動了明細不動
CREATE OR REPLACE FUNCTION public.require_voucher_year_if_multi_lot(
  p_member_id uuid,
  p_category text,
...
DECLARE
  ...
  v_any boolean;
BEGIN
  IF p_voucher_year IS NOT NULL THEN
    RETURN;
  END IF;
  IF p_category NOT IN (
    'vip_voucher', 'boat_voucher_g23', 'boat_voucher_g21_panther'
  ...
  SELECT COUNT(*) FILTER (WHERE remaining <> 0), COUNT(*) > 0
  INTO v_nonzero, v_any
  FROM credit_lots
  WHERE member_id = p_member_id
    AND category = p_category;

  IF COALESCE(v_nonzero, 0) > 1 THEN
    RAISE EXCEPTION
      '此會員有跨年度明細，請選擇入帳年後再儲存（不要選「無」）';
  END IF;

  IF COALESCE(v_any, false) AND COALESCE(v_nonzero, 0) = 0 THEN
    RAISE EXCEPTION
      '此會員已有年度明細，請選擇入帳年後再儲存（不要選「無」）';
  END IF;
END;
$$;

-- 新增記帳：無標年時與編輯同一套規則（剛好一年有剩額→自動；否則擋）
...
BEGIN
  ...
  IF p_category IN ('vip_voucher', 'boat_voucher_g23', 'boat_voucher_g21_panther') THEN
    IF p_adjust_type = 'increase' AND v_year IS NOT NULL THEN
      ...
      END IF;
    ELSIF p_adjust_type = 'increase' AND v_year IS NULL THEN
      -- 新增選無：剛好一年有剩額→自動寫入；有明細結構卻不能自動→擋下
      PERFORM public.require_voucher_year_if_multi_lot(
        p_member_id, p_category, NULL
      );
      v_resolved := public.resolve_credit_lot_year_for_untagged(p_member_id, p_category);
      IF v_resolved IS NOT NULL AND p_qty <> 0 THEN
        v_lot_ok := public.adjust_credit_lot_remaining(
          p_member_id, p_category, v_resolved, p_qty
        );
    ...
    IF v_tx.adjust_type = 'increase' AND v_tx.voucher_year IS NOT NULL THEN
      ...
      IF NOT COALESCE(v_lot_ok, false) THEN
        RAISE EXCEPTION
          '無法寫入年帳：年帳未開或與總額不一致。';
      END IF;
    ELSIF v_tx.adjust_type = 'increase' AND v_tx.voucher_year IS NULL THEN
      -- 無標年入帳：剛好一年用那一年；跨年刪除時退到最舊年（與 restore fallback 一致）
      v_resolved := public.resolve_credit_lot_year_for_untagged(p_member_id, v_tx.category);
      IF v_resolved IS NULL THEN
        SELECT MIN(voucher_year) INTO v_resolved
        FROM credit_lots
        WHERE member_id = p_member_id
          AND category = v_tx.category;
      END IF;
      IF v_resolved IS NOT NULL AND v_abs <> 0 THEN
        v_lot_ok := public.adjust_credit_lot_remaining(
          p_member_id, v_tx.category, v_resolved, -v_abs
        );
        IF NOT COALESCE(v_lot_ok, false) THEN
          RAISE EXCEPTION
            '無法寫入年帳：年帳未開或與總額不一致。';
...
BEGIN
  ...
  IF v_tx.category IN ('vip_voucher', 'boat_voucher_g23', 'boat_voucher_g21_panther') THEN
    IF v_tx.adjust_type = 'increase' AND v_tx.voucher_year IS NOT NULL THEN
      ...
      END IF;
    ELSIF v_tx.adjust_type = 'increase' AND v_tx.voucher_year IS NULL THEN
      -- 舊筆無標年：剛好一年用那一年；跨年退到最舊年（之後若新筆仍無標年會要求選年）
      v_resolved := public.resolve_credit_lot_year_for_untagged(p_member_id, v_tx.category);
      IF v_resolved IS NULL THEN
        SELECT MIN(voucher_year) INTO v_resolved
        FROM credit_lots
        WHERE member_id = p_member_id
          AND category = v_tx.category;
      END IF;
      IF v_resolved IS NOT NULL AND v_old_abs <> 0 THEN
        v_lot_ok := public.adjust_credit_lot_remaining(
          p_member_id, v_tx.category, v_resolved, -v_old_abs
        );
        IF NOT COALESCE(v_lot_ok, false) THEN
          RAISE EXCEPTION
            '無法寫入年帳：年帳未開或與總額不一致。不要年帳請選入帳年「無」。';
      ...
      END IF;
    ELSIF v_tx.adjust_type IS DISTINCT FROM 'increase' THEN
      -- 舊筆是減少：依 lot_allocations 精確加回（無明細則加回最舊年）
      PERFORM public.restore_credit_lots_from_allocations(
  ...
  IF p_category IN ('vip_voucher', 'boat_voucher_g23', 'boat_voucher_g21_panther') THEN
    ...
    ELSIF p_adjust_type = 'increase' AND v_year IS NULL THEN
      ...
      END IF;
    ELSIF p_adjust_type = 'decrease' THEN
      -- 新筆是減少：FIFO 從各年扣（不足時不擋，維持與日常扣款一致）
      v_fifo := public.try_consume_credit_lots_fifo(
        p_member_id, p_category, p_qty, v_after
      );
      IF COALESCE((v_fifo->>'applied')::boolean, false) THEN
  ...
  -- 保留原 transaction_type／booking_participant_id／shop_order_id（金流獨立）
...
GRANT EXECUTE ON FUNCTION public.resolve_credit_lot_year_for_untagged(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.require_voucher_year_if_multi_lot(uuid, text, integer) TO authenticated;
