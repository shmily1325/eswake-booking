...
| 項目 | 設定位置 | 欄位 | 計算方式 |
|------|----------|------|----------|
| 船隻儲值價格 | 船隻管理 | `balance_price_per_hour` | `Math.floor(price * minutes / 60)` |
| 船隻 VIP 價格 | 船隻管理 | `vip_price_per_hour` | `Math.floor(price * minutes / 60)` |
| 指定課價格 | 人員管理 | `designated_lesson_price_30min` | `Math.floor(price * minutes / 30)` |
...
**儲值金額計算（無條件捨去）：**
```javascript
金額 = Math.floor(balance_price_per_hour * 實際分鐘數 / 60)

例：balance_price_per_hour = 10800
    實際時長 = 60 分鐘
    金額 = Math.floor(10800 * 60 / 60) = 10,800 元
```
...
**指定課金額計算（無條件捨去）：**
```javascript
金額 = Math.floor(designated_lesson_price_30min * 實際分鐘數 / 30)

例：designated_lesson_price_30min = 1000
    實際時長 = 60 分鐘
    金額 = Math.floor(1000 * 60 / 30) = 2,000 元
```
...
#### 船隻費用（儲值/VIP）- 無條件捨去
```javascript
金額 = Math.floor(每小時價格 * 實際分鐘數 / 60)
```

**範例：**
| 設定 | 時長 | 計算過程 | 結果 |
|------|------|----------|------|
| $10,800/小時 | 30分 | floor(10800 * 30 / 60) | **$5,400** |
| $10,800/小時 | 40分 | floor(10800 * 40 / 60) | **$7,200** |
| $10,800/小時 | 60分 | floor(10800 * 60 / 60) | **$10,800** |
| $6,000/小時 | 20分 | floor(6000 * 20 / 60) | **$2,000** |

#### 指定課費用 - 無條件捨去
```javascript
金額 = Math.floor(30分鐘價格 * 實際分鐘數 / 30)
```

**範例：**
| 設定 | 時長 | 計算過程 | 結果 |
|------|------|----------|------|
| $1,000/30分 | 20分 | floor(1000 * 20 / 30) | **$666** |
| $1,000/30分 | 30分 | floor(1000 * 30 / 30) | **$1,000** |
| $1,000/30分 | 60分 | floor(1000 * 60 / 30) | **$2,000** |
...
function calculateBoatPrice(pricePerHour: number, minutes: number): number {
  return Math.floor(pricePerHour * minutes / 60)
}

// 指定課費用計算
function calculateDesignatedLessonPrice(price30min: number, minutes: number): number {
  return Math.floor(price30min * minutes / 30)
}
```
