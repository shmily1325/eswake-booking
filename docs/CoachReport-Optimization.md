...
**问题**: 用户需要熟悉新的添加方式，不够直观

**解决方案**:
1. 分离会员和非会员的添加按钮
   - "👤 搜尋並新增會員" → 创建会员状态参与者（`status: 'pending'`, 默认扣储值）
   - "✏️ 直接輸入客人姓名（非會員）" → 创建非会员状态参与者（`status: 'not_applicable'`, 默认现金）

2. 改进验证逻辑
   - 不再使用智能猜测会员
   - 明确检查：会员状态但无 `member_id` → 提示用户

3. 修复提交逻辑
   - 添加 try-catch 确保失败时不显示成功
   - 只有真正成功才显示"回報成功"
