...
| **E** | 新入帳可選／預設當年；扣款 FIFO **僅套用已有 lot 者**；細帳可看／改入帳年 | ⏳ `163`+`164` 已上線；建議再套 `165`（FIFO 失敗不擋日常扣款） |
...
- `credit_lots`（空表）
- `system_settings.current_voucher_year` = `2026`（舊版相容欄位；目前管理畫面依場地當年自動產生前一年／今年／下一年）
