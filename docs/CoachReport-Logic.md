...
### 1. **沒有教練，只有駕駛（純駕駛預約）** → `both`
   ...
   - **統計歸屬**：
     ...
     - 參與者時數 → 根據 `is_teaching` 欄位：
       - 指定課 → `is_teaching = true` ✅ 計入**教學時數**（如果駕駛本身也是教練）
       - 不指定 → `is_teaching = false` ❌ 不計入教學時數
