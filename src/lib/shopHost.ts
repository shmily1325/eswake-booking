...
/** shop 子網域沒有預約表；/book 轉到 book host */
export function shopToBookRedirectResponse(
  url: URL,
  bookHost: string,
): Response | null {
  const normalized = url.pathname.replace(/\/$/, '') || '/'
  if (normalized !== '/book') return null
  return Response.redirect(`https://${bookHost}/${url.search}`, 301)
}
