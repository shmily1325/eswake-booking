...
describe('getStepNextLabel', () => {
  ...
  it('returns ready labels when step is complete', () => {
    const { footer } = BOOK_I18N.zh
    expect(getStepNextLabel(1, footer, true)).toBe('下一步：填人數')
    expect(getStepNextLabel(2, footer, true)).toBe('下一步：選時間')
    expect(getStepNextLabel(3, footer, true)).toBe('下一步：看摘要')
  })
