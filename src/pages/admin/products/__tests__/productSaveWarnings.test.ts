...
describe('shouldWarnZeroStockOnSave', () => {
  it('warns on new SKU with stock 0 and no pre-order', () => {
    expect(
      shouldWarnZeroStockOnSave({ id: null, stock: '0', saleMode: 'standard' }, new Map()),
    ).toBe(true)
  })

  it('does not warn when pre-order is enabled', () => {
    expect(
      shouldWarnZeroStockOnSave({ id: null, stock: '0', saleMode: 'pre_order' }, new Map()),
    ).toBe(false)
  })

  it('does not warn for custom orders without stock', () => {
    expect(
      shouldWarnZeroStockOnSave({ id: null, stock: '0', saleMode: 'custom_order' }, new Map()),
    ).toBe(false)
  })

  it('does not warn when stock is positive', () => {
    expect(
      shouldWarnZeroStockOnSave({ id: 'v1', stock: '2', saleMode: 'standard' }, new Map([['v1', soldOutVariant]])),
    ).toBe(false)
  })
  ...
  it('does not warn when already sold out and unchanged', () => {
    ...
    expect(
      shouldWarnZeroStockOnSave({ id: 'v1', stock: '0', saleMode: 'standard' }, map),
    ).toBe(false)
  })
  ...
  it('warns when stock reduced to 0', () => {
    ...
    expect(
      shouldWarnZeroStockOnSave({ id: 'v2', stock: '0', saleMode: 'standard' }, map),
    ).toBe(true)
  })
})
...
describe('collectZeroStockWarnings', () => {
  it('skips pending delete drafts', () => {
    const warnings = collectZeroStockWarnings(
      [{ id: null, stock: '0', saleMode: 'standard', pendingDelete: true }],
      new Map(),
    )
